...
void DispatchClientCommand(CommandSender * apCommandObj, CommandId aCommandId, EndpointId aEndpointId, TLV::TLVReader & aDataTlv)
{
    ...
    bool wasHandled              = false;
    {
        switch (aCommandId)
        {
        case Clusters::TimeSync::Commands::Ids::SetUtcTime: {
            expectArgumentCount = 3;
            uint64_t UtcTime;
            uint8_t Granularity;
            uint8_t TimeSource;
            ...
            while ((TLVError = aDataTlv.Next()) == CHIP_NO_ERROR)
            {
                ...
                {
                case 0:
                    TLVUnpackError = aDataTlv.Get(UtcTime);
                    break;
                case 1:
                    TLVUnpackError = aDataTlv.Get(Granularity);
                    break;
                case 2:
                    TLVUnpackError = aDataTlv.Get(TimeSource);
                    break;
                default:
                    // Unsupported tag, ignore it.
                    ChipLogProgress(Zcl, "Unknown TLV tag during processing.");
            ...
            if (CHIP_NO_ERROR == TLVError && CHIP_NO_ERROR == TLVUnpackError && 3 == validArgumentCount)
            {
                wasHandled = emberAfTimeSyncClusterSetUtcTimeCallback(aEndpointId, apCommandObj, UtcTime, Granularity, TimeSource);
            }
            break;
        }
        default: {
            // Unrecognized command ID, error status will apply.
            ReportCommandUnsupported(apCommandObj, aEndpointId, Clusters::TimeSync::Id, aCommandId);
            return;
        }
        }
    }
    ...
    if (CHIP_NO_ERROR != TLVError || CHIP_NO_ERROR != TLVUnpackError || expectArgumentCount != validArgumentCount || !wasHandled)
    {
        CommandPathParams returnStatusParam = { aEndpointId,
                                                0, // GroupId
                                                Clusters::TimeSync::Id, aCommandId, (CommandPathFlags::kEndpointIdValid) };
        apCommandObj->AddStatusCode(returnStatusParam, Protocols::SecureChannel::GeneralStatusCode::kBadRequest,
                                    Protocols::SecureChannel::Id, Protocols::InteractionModel::ProtocolCode::InvalidCommand);
        ChipLogProgress(Zcl,
                        "Failed to dispatch command, %" PRIu32 "/%" PRIu32 " arguments parsed, TLVError=%" CHIP_ERROR_FORMAT
                                          ...
                                          TLV::TLVReader & aReader, CommandSender * apCommandObj)
{
    ...
    case Clusters::TestCluster::Id:
        ...
        break;
    case Clusters::TimeSync::Id:
        clusters::TimeSync::DispatchClientCommand(apCommandObj, aCommandId, aEndPointId, aReader);
        break;
    default:
        // Unrecognized cluster ID, error status will apply.
        CommandPathParams returnStatusParam = { aEndPointId,
